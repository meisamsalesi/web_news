            ...
            foreach ($posts as $post) {
                ...
                <tr>
                    ...
                    <td style="width: 25rem;">
                        <a role="button"
                            class="btn btn-sm btn-warning <?= $post['breaking_news'] == 1 ? 'btn-dark' : '' ?> text-white"
                            href="<?= url('admin/post/breaking-news/' . $post['id']) ?>">
                            <?= $post['breaking_news'] == 1 ? 'remove breaking news' : 'add breaking news' ?>
                        </a>
                        <a role="button"
                            class="btn btn-sm btn-warning <?= $post['selected'] == 1 ? 'btn-dark' : '' ?> text-white"
                        ...
                        <a role="button"
                            class="btn btn-sm btn-warning <?= $post['status'] == 'enable' ? 'btn-dark' : '' ?> text-white"
                            href="<?= url('admin/post/status/' . $post['id']) ?>">
                            <?= $post['status'] == 'enable' ? 'disable' : 'enable' ?>
